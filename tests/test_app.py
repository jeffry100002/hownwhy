...
from app import app, text_model, image_model
...
class ChatbotAppTestCase(unittest.TestCase):
    ...
    def test_ai_suggests_image_and_image_succeeds(self, mock_image_gen, mock_text_gen):
        ...
        type(mock_text_candidate.content).parts = PropertyMock(return_value=[mock_text_part]) # ensure parts is a list-like mock
        type(mock_text_response_obj).candidates = [mock_text_candidate]
        type(mock_text_response_obj).prompt_feedback = PropertyMock(return_value=None) # ensure prompt_feedback is None or evaluates to False
        ...
        type(mock_image_candidate.content).parts = PropertyMock(return_value=[img_resp_text_part, img_resp_image_part]) # ensure parts is a list-like mock
        type(mock_image_response_obj).candidates = [mock_image_candidate]
        type(mock_image_response_obj).prompt_feedback = PropertyMock(return_value=None) # ensure prompt_feedback is None or evaluates to False
        ...
        # MODIFICATION HERE: image_model.generate_content is now called without generation_config
